...
<div x-data="{ open: false,
    isValid: { name: true, description: true, img_url: true, price: true, discount: false, weight: true },
    inputValues: { name: '', description: '', img_url: '', price: '', discount: '', weight: '', is_available: false },
    submit() {
      // this.validateName();
    ...
    validateDiscountAmount() {
      ...
      this.isValid.discount = this.inputValues.discount.trim() !== '';
    },
    validateWeight() {
      this.isValid.weight = this.inputValues.weight.trim() !== '';
    },
 }"
 x-on:update-item-form-open.window="
    document.getElementById('update-heading').innerText = 'Update Item ' + updateItem.name;
    inputValues.name = updateItem.name;
    inputValues.description = updateItem.description;
    inputValues.img_url = updateItem.imgUrl;
    inputValues.price = updateItem.price;
    inputValues.discount = updateItem.discount;
    inputValues.weight = updateItem.weight;
    inputValues.is_available = updateItem.is_available;
 "
>

    <div x-show="updateItemFormOpen" class="fixed top-0 left-0 w-full h-full z-40 bg-black bg-opacity-80">
        <div class="fixed inset-0 items-center justify-center mt-24 max-w-2xl mx-auto overflow-x-hidden overflow-y-auto z-50">
            <div class="relative max-w-2xl mx-auto">
                <div class="bg-white w-full p-6 text-wrap break-words flex flex-col" style="max-height: 80vh; overflow-y: auto">
                    <h2 id="update-heading" class="text-2xl font-bold mb-4 text-center">Update Item</h2>
                    <form @submit.prevent="submit" action="/items" method="POST">
                        ...
                        <div class="flex flex-col mb-4">
                            <x-form.input-label for="discount">Discount By Percentage:</x-form.input-label>
                            <x-form.text-input @blur="validateDiscountPrice" placeholder="Item's Discounted Price" name="discount" x-model="inputValues.discount" />
                            {{-- <p x-show="!isValid.discount" class="text-red-600 text-sm mt-1" id="name-required" x-cloak>Item's discounted price required.</p> --}}
                        </div>
                        ...
                        <div class="flex justify-center space-x-4">
                            <button type="button" class="bg-red-500 text-white rounded-lg px-6 py-2" @click="updateItemFormOpen = false">
                                Close
                            </button>
