...
<div x-data="{ open: false,
    isValid: { name: true, description: true, img_url: true, price: true, old_price: true, weight: true },
    ...
    validateOldPrice() {
      this.isValid.old_price = this.inputValues.old_price.trim() !== '';
    },
    validateWeight() {
            ...
            <div class="relative w-auto max-w-2xl mx-auto">
                <div class="bg-white w-full p-6 text-wrap break-words flex flex-col" style="max-height: 80vh; overflow-y: auto">
                    ...
                    <form @submit.prevent="submit" action="/restaurants/{{ $restaurant->id }}/menu/{{ $category->id }}/items" method="POST">
                        ...
                        <div class="flex flex-col mb-4">
                            <x-form.input-label for="old_price">Item Old Price:</x-form.input-label>
                            <x-form.text-input @blur="validateOldPrice" placeholder="Item's Old Price" name="old_price" x-model="inputValues.old_price" />
                            <p x-show="!isValid.old_price" class="text-red-600 text-sm mt-1" id="name-required" x-cloak>Item's old price required.</p>
                        </div>
                        <div class="flex flex-col mb-4">
